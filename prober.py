...
def GetDetails(c):
    ...
    diffc = Page_O.find_all('img',attrs={"class": "playlog_diff v_b"})[0].get('src').split('/')[-1].split('.')[0].split('_')[1]
    typec  = Page_O.find_all('img',attrs={"class": "playlog_music_kind_icon"})[0].get('src').split('/')[-1].split('.')[0].split('_')
    if(typec[1]=="standard"):
        typec="SD"
    else:
        typec="DX"
    title = Page_O.find_all('div',attrs={"class": "basic_block m_5 p_5 p_l_10 f_13 break"})[0].get_text()
    achievement =  Page_O.find_all('div',attrs={"class": "playlog_achievement_txt t_r"})[0].get_text()
    dx_score = Page_O.find_all('div',attrs={"class": "white p_r_5 f_15 f_r"})[0].get_text()
    Stc = RecordStatus(Page_O.find_all('img',attrs={"class": "h_35 m_5 f_l"}))
    FLstat = Page_O.find_all('div',attrs={"class": "p_t_5"})
    Ratings = Page_O.find_all('div',attrs={"class": "playlog_rating_val_block"})
    Fsfc = Page_O.find_all('div',attrs={"class": "f_r f_14 white"})
    TAPStat = RecordStat(Page_O.find_all('table',attrs={"class": "playlog_notes_detail t_r f_l f_11 f_b"})[0].find_all('tr')[1:6])
    ...
    try:
        ...
        rank = Page_O.find_all('img',attrs={"class": "h_35 m_5 f_r"})[0].get('src').split('/')[-1].split('.')[0]
        ppid  = Page_O.find_all('div',attrs={"class": "basic_block p_3 t_c f_11"})[0].get_text()
        ppidlevel = Page_O.find_all('div',attrs={"id": "matching"})
        cd=ppidlevel[0].find_all('img',attrs={"class": "h_16"})
        ppidlevel=cd[0].get('src').split('/')[-1].split('.')[0].split('_')[1].upper()
    except Exception:
        rank="N/A"
        fsd=['-','-']
    ResList=[]
    ...
    ResList.append(str(int(Ratings[0].get_text()) + int(Ratings[1].get_text())))
    ResList.append('3')
...
class MainHandler(tornado.web.RequestHandler):
    def get(self):
        ...
        for i in range(5):
            sleep(0.2)
            Netx = rc.get("https://maimai.wahlap.com/maimai-mobile/record/musicGenre/search/?genre=99&diff={}".format(i))
            with open("indexc{}".format(i),"w") as c:
                c.write(Netx.content.decode("utf8"))
        
        Netx = rc.get("https://maimai.wahlap.com/maimai-mobile/record/".format(i))
        Page_O=BeautifulSoup(Netx.content.decode('utf8'),'html.parser')
        res=Page_O.find_all('input',attrs={"name": "idx"})
        ResList=[]
        for i in res:
            print(i.get("value"))
            cc=rc.get("https://maimai.wahlap.com/maimai-mobile/record/playlogDetail/?idx={}".format(i.get("value"))).content.decode('utf8')
...
class RedirectHandler(tornado.web.RequestHandler):
    def get(self):
        self.redirect("http://tgk-wcaime.wahlap.com/wc_auth/oauth/authorize/maimai-dx")

class Authorizehandler(tornado.web.RequestHandler):
	def get(self):
		resc = requests.get("https://tgk-wcaime.wahlap.com/wc_auth/oauth/authorize/maimai-dx",headers=headerc,allow_redirects=False)
...
def updateSongList():
    ...
    musicData = requests.get("https://www.diving-fish.com/api/maimaidxprober/music_data")
